...
export const Component = () => {
  // Pause the WebGL render loop while the tab is hidden — this shader runs a
  // 50-iteration-per-pixel raymarch every frame at full framerate (unlike
  // the pixel-stars canvas it replaces, which was already throttled to
  // 16fps), so leaving it spinning in a background tab is a real battery
  ...
  return (
    <div className={cn("flex flex-col items-center gap-4 p-0 rounded-lg w-full h-full")}>
      <Canvas orthographic camera={{ position: [0, 0, 1], zoom: 1 }} dpr={[1, 2]} frameloop={frameloop}>
        {/* Warm near-black rather than pure black — this canvas is now the
            site's permanent backdrop in both themes (see App.jsx), and a
            slightly warm void reads as intentional behind the light-mode
            cream UI instead of a stark black hole, while still being dark
            enough for the gold streaks to glow properly. */}
